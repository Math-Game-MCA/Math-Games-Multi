App.controller('NewRuleController', [ '$scope', 'Restangular', '$rootScope', '$state', '$stateParams', 'flash', function($scope, Restangular, $rootScope, $state, $stateParams, flash) {
	'use strict';
	...
	$scope.selectAllValues = selectAllValues;
	$scope.cancel = cancel;
	$scope.save = save;
	$scope.rules = {};
	function rules() {		
		Restangular.one('new-rules', $stateParams.accountId).get().then(function(data){
	        $scope.rules = data.data;
	        $scope.rules.ruleName = 'Rule #' + (data.cnt);
	      }, function (err) {
      	});
	}
	...
	function addFilters(idx) {
	    ...
	    $scope.rules.buttons[idx]['hide'] = true;//anurag
	}
    ...
    function selectAllValues(idx) {
    	angular.forEach($scope.rules.filters[idx].list, function(k, v) {
    		
    		setSelectionValues(idx, '',k);
    	});
    }
